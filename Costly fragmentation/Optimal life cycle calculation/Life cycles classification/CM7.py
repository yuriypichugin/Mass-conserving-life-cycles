...
v6.2 added proportional loss optimization
v7 changed LambdaDiff calculation to the interpolation algorithm
"""
...
def LambdaDiff_Interpolate(L, Fragments, Loss, X, T):
	"""
...
#	ParentPosition = np.argmin(abs(X - ParentSize))
	ParentPosition = np.interp(ParentSize, X, np.arange(len(X)))
	T_Parent = np.interp(ParentPosition, np.arange(len(T)), T)
	Diff = 1
	
	for frag in Fragments:
#		FragPosition = np.argmin(abs(X - frag))
		FragPosition = np.interp(frag, X, np.arange(len(X)))
		T_Frag = np.interp(FragPosition, np.arange(len(T)), T)
		Diff -= np.exp(  L*(T_Frag - T_Parent)  )
	return Diff
...
#	Lmin = min(np.divide(G,X+1e-6))
	...
	DiffMin=np.sign(LambdaDiff_Interpolate(Lmin, Fragments, Loss, X, T))
	DiffMax=np.sign(LambdaDiff_Interpolate(Lmax, Fragments, Loss, X, T))
	while DiffMax<0:
		Lmax = Lmax*2
		DiffMax=np.sign(LambdaDiff_Interpolate(Lmax, Fragments, Loss, X, T))
	...
	if(DiffMin*DiffMax>0):
		...
		while(Lmax - Lmin > Laccuracy):
			Lmid = (Lmax+Lmin)/2.0
			DiffMid=np.sign(LambdaDiff_Interpolate(Lmid, Fragments, Loss, X, T))
...
def XGT_generate(XParams, GParams, GenerationMethod):
	...
	for i in np.arange(1,len(T)):
		T[i] = T[i-1] + (X[i]-X[i-1])/((G[i]+G[i-1])/2.0)
	return [X, G, T]
...
def G_generate(x, Parameters):
	...
	if Model == 1:
		...
		"""
		Model 19 is the linear interpolation through the list of nodes
		Arguments:
		...
		"""
		Model 20 is the logarithmic interpolation through the list of nodes 
		(linear in a log scale over X)
		Arguments:
			(list of floats) Params[0] - log10(X)-coordinates of the nodes
			(list of floats) Params[1] - Y-coordinates of the nodes
		"""
		log10xnodes = Parameters[1]
		ynodes = Parameters[2]
		...
		count = 0
		for X in x:
			ind = sum(np.log10(X) > log10xnodes)
			y = (np.log10(X) - log10xnodes[ind-1])/(log10xnodes[ind] - log10xnodes[ind-1])*(ynodes[ind] - ynodes[ind-1])+ynodes[ind-1]
			b[count] = y
			count+=1
		""" normalization """
		b = b / sum(b)
		...
		"""
		Model 21 is the logarithmic interpolation through the list of nodes 
		(linear in a log scale over X)
		This is another implementation, in which X-coordinates are supplied in their natural form
		Arguments:
			(list of floats) Params[0] - X-coordinates of the nodes
			(list of floats) Params[1] - Y-coordinates of the nodes
		"""
		...
		"""
		Model 22 is the exponential interpolation through the list of nodes 
		(linear in a log scale over Y)
		Arguments:
			(list of floats) Params[0] - X-coordinates of the nodes
			(list of floats) Params[1] - Y-coordinates of the nodes
		"""
...
#		log10xnodes = np.log10(xnodes)
		...
		for X in x:
			ind = sum(X > xnodes)
			log10y = (X-xnodes[ind-1])/(xnodes[ind] - xnodes[ind-1])*(log10ynodes[ind] - log10ynodes[ind-1])+log10ynodes[ind-1]
			b[count] = np.power(10, log10y)
			count+=1
		""" normalization """
		b = b / sum(b)
	elif Model == 23:
		"""
		Model 23 is the power law interpolation through the list of nodes 
		(linear in a log scale over X and Y)
		Arguments:
			(list of floats) Params[0] - X-coordinates of the nodes
			(list of floats) Params[1] - Y-coordinates of the nodes
		"""
		...
		for X in x:
			ind = sum(X > xnodes)
			log10y = (np.log10(X)-log10xnodes[ind-1])/(log10xnodes[ind] - log10xnodes[ind-1])*(log10ynodes[ind] - log10ynodes[ind-1])+log10ynodes[ind-1]
			b[count] = np.power(10, log10y)
			count+=1
		""" normalization """
		b = b / sum(b)
	elif Model == 24:
		""" Model 24 is gaussian bell with the minimal base value"""
		x_0 = Parameters[1]
		sigma = Parameters[2]
		prefactor = Parameters[3]
		BaseFactor = Parameters[4]
		Gauss = prefactor*np.exp( -np.power((x-x_0),2)/(2.0*sigma*sigma) )
		b = np.clip(Gauss, BaseFactor, max(Gauss))
	else:
...
def OptimizeGrowth_seeded(XParams, GParams, Loss, Perturbation, NumofOffspring, StartFRG):
	"""
	Find the optimal fragmentaiton mode for given environment
	Arguments:
		(list) XParams - the set of parameters to generate X-array
		...
		(float) Loss - the amount of biomass, lost at the fragmentation
		(float) Perturbation - the change in the offspring size (should be greater than the step in X)
		(int) NumofOffspring - NOT USED! the number of offspring (at Loss = 0, this should be 2)
		(list) StartFRG - the fragmentation mode from which the optimization starts (should comply with NumofOffspring)
	Return:
		(list) Output
			Output[0] - the optimal number of offspring
			Output[1] - the optimal life cycle, list of the offspring sizes
...
""" ------- Proportional loss -------- """
def OptimizeGrowth_PropLoss(XParams, GParams, LossFrac, Perturbation, NumofOffspring, StartFRG):
	"""
	Find the optimal fragmentaiton mode for given environment
	Arguments:
		(list) XParams - the set of parameters to generate X-array
		(list) GParams - the set of parameters to generate G-array
		(float) LossFrac - the fraction of biomass, lost at the fragmentation
		(float) Perturbation - the change in the offspring size (should be greater than the step in X)
		(int) NumofOffspring - NOT USED! the number of offspring (at Loss = 0, this should be 2)
		(list) StartFRG - the fragmentation mode from which the optimization starts (should comply with NumofOffspring)
	Return:
...
#	bestFRG = max(GParams[1], 1.1) * np.ones(2)
	bestFRG = StartFRG
	CurrentLoss = np.sum(bestFRG)*LossFrac/(1.0 - LossFrac)
	bestLambda = LambdaBisect(bestFRG, CurrentLoss, X, G, T)
	...
	while TheBestFound == 0:
		...
		for index in np.arange(len(bestFRG)):
			...
			testFRG = np.array(bestFRG)
			testFRG[index] += Perturbation
			TestLoss = np.sum(testFRG)*LossFrac/(1.0 - LossFrac)
			testLambda = LambdaBisect(testFRG, TestLoss, X, G, T)
			if testLambda > CurrentBestLambda:
				CurrentBestFRG = np.array(testFRG)
				CurrentBestLambda = testLambda
				TheBestFound=0

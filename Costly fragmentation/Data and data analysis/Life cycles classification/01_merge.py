...
#
##	FileList = []
#	DataPath = FolderName + DataFileMask
##	ErrorPath = FolderName+'errors/error.LC_'
#
	Index = 0
	for Data_file in FileList:
#		ErrorFile = ErrorPath+str(i)
		FileName = FolderName+Data_file
		if (os.stat(FileName).st_size > 0):
			
			FileData = pd.read_csv(FileName, delimiter = ', ', engine='python')
			if Index == 0:
				LastEntryData = FileData
			else:
				LastEntryData = pd.concat([LastEntryData, FileData], ignore_index=True, axis = 0)
			Index+=1
...
FolderName = 'Raw data/'
File2WriteName = 'Costly_Gauss_Loss_Sigma_screen_calc5.txt'
DataFileMask = 'CostlyGaussian_MaxBenefit'
